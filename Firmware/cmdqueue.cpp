...
static bool process_char(const char serial_char) {
...
#ifdef USE_DIRECT_SERIAL_RX
static uint8_t serial_drain_func(const uint8_t c) {
    process_char((char)c);
}

// Latch serial drain above
void cmdqueue_init() {
    MYSERIAL.setDrainFunction(&serial_drain_func);
}
#endif
...
void get_command()
{
  ...
  while (((MYSERIAL.available() > 0 && !saved_printing) || (MYSERIAL.available() > 0 && isPrintPaused)) && !cmdqueue_serial_disabled) {  //is print is saved (crash detection or filament detection), dont process data from serial line
      ...
      if (!process_char(MYSERIAL.read())) return;
