...
do
	...
	# set FW_REPOSITORY
	sed -i -- 's/#define FW_REPOSITORY "Unknown"/#define FW_REPOSITORY "ScottMudge"/g' $SCRIPT_PATH/Firmware/Configuration.h
...
#	fi	
	...
	sed -i -- 's/^#define FW_REPOSITORY "ScottMudge"/#define FW_REPOSITORY "Unknown"/g' $SCRIPT_PATH/Firmware/Configuration.h
	if [ ! -z "$BUILD_ORG" ] ; then
